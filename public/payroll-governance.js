(function () {
  ...
  function effectiveAdjustments(emp,period) {
    return PAYROLL_ADJ.filter(function (adjustment) {
      if(adjustment.empId!==emp.id||!isPayrollAdjustmentReady(adjustment))return false;
      ...
      return Object.assign({},adjustment,{taxable:payItem?payItem.taxable:adjustment.amount>0,category:payItem?payItem.cat:adjustment.category,benefitTreatment:payItem&&payItem.benefitTreatment||adjustment.benefitTreatment||''});
    });
  }
  ...
  function governanceDraft(emp,grp,period) {
    ...
    var result=PayrollRuleEngine.calculate({employee:appliedEmp,group:grp,period:p,attendance:attendance,rules:PAYROLL_RULEBOOK,baseBasic:baseBasic,defaultDivisor:COMPANY.dailyDivisor||22,periodDays:(typeof workingDaysInPeriod==='function'?workingDaysInPeriod(p):COMPANY.dailyDivisor||22),basicPayProrationPolicy:COMPANY.basicPayProrationPolicy,recurringAllowances:recurringAllowances,adjustments:adjustments,annualBenefitContext:{limit:90000,previousEmployerNonTaxable:Number(taxProfile.previousEmployerNonTaxableBenefits||0),currentEmployerYtdNonTaxable:benefitYtd.exempt},loans:LOANS.filter(function(loan){return loan.eid===emp.id;}),statutory:statutoryAmounts,tax:birTaxByFreq,otRates:(typeof OT_RATES==='undefined'?[]:OT_RATES),otOverrideAmount:otOverrideAmount});
  ...
  window.runPayroll=runPayroll=function(){
    ...
    if(pendingRun){pendingRun.status='superseded';pendingRun.supersededAt=now;pendingRun.supersededBy=user.name;pendingRun.supersededReason='Replacement payroll submitted after ready adjustments';PAYROLL_AUDIT.push({runId:pendingRun.id,action:'superseded_for_reprocess',by:user.name,at:now,notes:'Replacement payroll was submitted from the adjustment reprocessing flow'});}
    var sourcePeriod=PAY_PERIODS.find(function(period){return period.id===(window._prPeriod!=='custom'?window._prPeriod:null);});
    var paymentDate=(sourcePeriod&&sourcePeriod.releaseDate)||to;
  ...
  function validationInventory(){
    var issues=[];USERS.filter(function(e){return e.role==='employee';}).forEach(function(emp){PayrollRuleEngine.validateEmployee(emp).forEach(function(issue){issues.push(Object.assign({employee:fmtEmpName(emp)},issue));});});
  ...
  function componentsView(){
    var taxable=INCOME_TYPES.filter(function(i){return i.taxable;}).length;
    var deminimis=INCOME_TYPES.filter(function(i){return i.deminimis;}).length;
    var annualized=INCOME_TYPES.filter(function(i){return i.annualize;}).length;
    var byCat={};INCOME_TYPES.forEach(function(i){byCat[i.cat]=(byCat[i.cat]||0)+1;});
    return '<div class="card"><div class="card-hd"><div><div class="card-title">Pay Component Classification</div><div class="card-sub">Names never determine tax treatment; each code carries explicit flags. Add, edit, or delete components in Settings.</div></div><button class="btn btn-sm btn-primary" onclick="goTab(7);window._cfgSub=2;render()">Manage components →</button></div>'+
    '<div class="metrics" style="margin:0 0 12px"><div class="metric"><div class="metric-label">Total Components</div><div class="metric-val" style="font-size:20px">'+INCOME_TYPES.length+'</div></div><div class="metric"><div class="metric-label">Taxable</div><div class="metric-val" style="font-size:20px">'+taxable+'</div></div><div class="metric"><div class="metric-label">De Minimis</div><div class="metric-val" style="font-size:20px">'+deminimis+'</div></div><div class="metric"><div class="metric-label">Annualized</div><div class="metric-val" style="font-size:20px">'+annualized+'</div></div></div>'+
    '<div style="display:flex;flex-wrap:wrap;gap:6px">'+Object.keys(byCat).sort().map(function(cat){return '<span class="badge b-info">'+esc(cat)+' · '+byCat[cat]+'</span>';}).join('')+'</div>'+
    '</div>';
  }
  function calendarView(){
    var openCount=PAY_PERIODS.filter(function(p){return p.status==='open';}).length;
    var lockedCount=PAY_PERIODS.filter(function(p){return p.status==='locked';}).length;
    var upcoming=PAY_PERIODS.filter(function(p){return p.status==='open'||p.status==='processing';}).slice(0,3);
    return '<div class="card"><div class="card-hd"><div><div class="card-title">Legal Entity Payroll Identity</div><div class="card-sub">Government registrations and banking remain maintained in Company Settings.</div></div><button class="btn btn-sm" onclick="goView(\'company\')">Open Company Settings</button></div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px"><div><div class="metric-label">Registered Name</div><strong>'+esc(COMPANY.registeredName||COMPANY.name||'—')+'</strong></div><div><div class="metric-label">TIN</div><strong>'+esc(COMPANY.tin||'—')+'</strong></div><div><div class="metric-label">RDO</div><strong>'+esc(COMPANY.rdo||'—')+'</strong></div><div><div class="metric-label">Currency</div><strong>'+esc(COMPANY.currency||'PHP')+'</strong></div></div></div>'+
    '<div class="card"><div class="card-hd"><div><div class="card-title">Payroll Calendar and Cutoff Controls</div><div class="card-sub">Add periods, lock, unlock, or reopen them in Settings.</div></div><button class="btn btn-sm btn-primary" onclick="goTab(7);window._cfgSub=1;render()">Manage calendar →</button></div>'+
    '<div class="metrics" style="margin:0 0 12px"><div class="metric"><div class="metric-label">Total Periods</div><div class="metric-val" style="font-size:20px">'+PAY_PERIODS.length+'</div></div><div class="metric"><div class="metric-label">Open</div><div class="metric-val" style="font-size:20px;color:var(--green)">'+openCount+'</div></div><div class="metric"><div class="metric-label">Locked</div><div class="metric-val" style="font-size:20px">'+lockedCount+'</div></div></div>'+
    (upcoming.length?'<div style="overflow-x:auto"><table><thead><tr><th>Period</th><th>Pay Date</th><th>Lock Date</th><th>Status</th></tr></thead><tbody>'+upcoming.map(function(period){return '<tr><td><strong>'+esc(period.label)+'</strong><div class="mono" style="font-size:10px">'+period.from+' → '+period.to+'</div></td><td><strong>'+period.releaseDate+'</strong></td><td>'+period.lockDate+'</td><td><span class="badge '+(period.status==='open'?'b-approved':period.status==='locked'?'b-pending':'b-info')+'">'+esc(period.status)+'</span></td></tr>';}).join('')+'</tbody></table></div>':'<div class="empty-state">No open or in-progress pay periods.</div>')+
    '</div>';
  }
